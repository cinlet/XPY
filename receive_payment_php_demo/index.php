<?php
	...
	function process_form() {
		...
		// Calculate the amount to charge (in TTC)
		$paycoins = 0;
		...
		for( $position = 0; $position < count($products); ++$position ) {
			...
			if( $quantity > 0 ) {
				$paycoins = $paycoins + $quantity * $prices[$position];
			}
		}
		...
		// Create a new order
		$mySQL->insert( "${DB_TABLE_PREFIX}orders", array( "amount"=>$paycoins ) );
		...
		// URL to call the API
		$api_call = $API . "receive.php?method=create&address=$MY_ADDRESS&callback=" . urlencode($callback_url);
